...
if (isset($_POST['post'])){
    ...
    $member         = $_SESSION['id'];
    $bankaccntName  = $_POST['bankaccntName'];
    $bankname       = $_POST['bankname'];
    $bankaccount    = $_POST['bankaccount'];

     
    $sql = "UPDATE members SET bankaccntName='$bankaccntName', bankname='$bankname', bankaccount='$bankaccount' WHERE memberID='$member'";
...
if(strlen($row['bankaccntName']) > 0){
    echo '<div class="card"><div class="card-body">';
    echo "Bank Account is saved, Contact the nearest DTEHM center to change bank account details";
    echo '</div></div>';
}elseif($row['bankaccntName'] < 1){
...
<?php 
    ...
    $result = mysqli_query($con, $sql);
    $row = mysqli_fetch_array($result);
    echo "<p>Bank Account Name</p>"."<p>".$row['bankaccntName']."</p>";
    echo "<p>Account Number</p>"."<p>".$row['bankaccount']."</p>";
    echo "<p>Bank Name</p> "."<p>".$row['bankname']."</p>";
        ?>
</div>
</div>
</div>
